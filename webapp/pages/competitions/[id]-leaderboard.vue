...
// 从路由参数中提取真实的比赛ID（移除 -leaderboard 后缀）
const competitionId = computed(() => {
  const idParam = route.params.id as string;
  return idParam?.replace("-leaderboard", "") || "";
});

// 添加调试日志
console.log("[LEADERBOARD PAGE] 页面加载，当前路由:", route.path);
console.log("[LEADERBOARD PAGE] 原始参数:", route.params.id);
console.log("[LEADERBOARD PAGE] 解析后的比赛ID:", competitionId.value);
console.log("[LEADERBOARD PAGE] 完整路径:", route.fullPath);
...
const { data: leaderboardData, pending, error } = useFetch<TeamLeaderboardEntry[]>(
  () => `/api/competitions/${competitionId.value}/leaderboard`
);
...
const {
  ...
  error: problemsError,
} = useFetch<ProblemsResponse>(() => `/api/competitions/${competitionId.value}/problems`);
