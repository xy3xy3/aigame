      ...
      >
        ...
        <div class="p-6">
          ...
          <div class="flex space-x-2">
            ...
            <div
              v-else-if="competition.status === 'ongoing' && competition.userParticipating"
...
const availableTeams = computed(() => {
  ...
  return teamsData.value.teams.filter(team =>
    !team.isLocked &&
    !team.participatingIn?.includes(selectedCompetition.value.id)
  )
})
...
const confirmJoinCompetition = async () => {
  ...
  try {
    const response = await $fetch(`/api/competitions/${selectedCompetition.value.id}/join`, {
      method: 'POST',
      body: {
