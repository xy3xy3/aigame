<template>
  <div class="max-w-4xl mx-auto py-6 px-4">
    ...
    <!-- 编辑表单 -->
    <form v-else-if="data?.problem" @submit.prevent="handleSubmit" class="bg-white rounded-lg shadow-md p-6 space-y-6">
      ...
      <div v-if="success" class="bg-green-50 border border-green-200 rounded-md p-4">
        <p class="text-green-800">题目更新成功！</p>
      </div>
...
// 获取题目数据
const { data, pending, error } = await useFetch<ProblemResponse>(`/api/problems/${problemId}`)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    submitError.value = err.data?.message || err.message || '更新题目失败'
  } finally {
